...
import { Edit, UserPlus, Clock, Trash2, MessageSquare, Plus, Minus } from 'lucide-react';

// --- Infynd Design System Colors ---
const INFIND_BLUE = "#007AFF"; 

export const LinkedInNode = ({ step, onEdit, onDelete }) => {
    const [isExpanded, setIsExpanded] = useState(false);
    const [currentDelay, setCurrentDelay] = useState(step.delay);

    const isConnect = step.type === 'invitation';
    const title = step.action || (isConnect ? "LinkedIn Connect Request" : "LinkedIn Message");
    const Icon = isConnect ? UserPlus : MessageSquare;
    
    const defaultMessage = step.message || "Hi {{FirstName}}, I wanted to reach out...";
    ...
    const renderExpandedView = () => {
        return (
            <div className="p-4 w-full max-w-lg bg-white shadow-xl rounded-lg border border-gray-200">
                ...
                {/* Message Editor (Omitted for brevity) */}
                ...
                <div className="flex justify-end pt-4">
                    ...
                    <Button 
                        style={{backgroundColor: INFIND_BLUE}}
                        className={`text-white hover:bg-blue-600`}
                        onClick={() => { 
                            onEdit({...step, message: defaultMessage, delay: currentDelay}); 
                            setIsExpanded(false); 
                        }}
                    >
                        Save
